...
# Ideally it should take in the assembly file of all of your samples, followed by the *BAM FILES* of all the samples that went into the assembly.
...
# 
# This script was modified from binning.sh by Bryan Merrill.
# 
##############################################################################################################################################################
...
help_message () {
	echo ""
	echo "Usage: metaWRAP binning_bams [options] -a assembly.fa -o output_dir readsA.bam readsB.bam ... [readsX.bam]"
...
num_of_bam_files=$(for I in "$@"; do echo $I | grep .bam; done | wc -l)
...
#	fi	
#done
#Making symlinks of bam files in work directory
num_of_bam_files=$(for I in "$@"; do echo $I | grep .bam; done | wc -l)

for I in "$@"; do
	basename=`echo ${I} | sed 's\.*/\\\g'`
	if [[ $I == *".bam"* ]]; then
		ln -s ../../${I} ${out}/work_files/${basename}
	fi
done
